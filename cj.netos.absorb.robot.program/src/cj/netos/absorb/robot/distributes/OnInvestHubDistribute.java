...
public class OnInvestHubDistribute implements IHubDistribute<InvestRecord> {
    ...
    public void distribute(InvestRecord result) throws CircuitException {
        ...
        //每权的价格
        BigDecimal weightPricePerAbsorber=new BigDecimal(result.getAmount()+"").divide(absorber.getWeight(), RobotUtils.BIGDECIMAL_SCALE,RoundingMode.HALF_DOWN);
        ...
        if (remainingAmount.compareTo(new BigDecimal("0.00")) > 0) {
            ...
            absorberHubService.addTailAmount(remainingAmount,result.getInvester(),absorber.getBankid(),result.getSn(),1, "尾金");
        }
    }
}
